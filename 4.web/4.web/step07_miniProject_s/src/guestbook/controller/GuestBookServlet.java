...
public class GuestBookServlet extends HttpServlet {
	...
	private void delete(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			if (GuestBookDAO.deleteContent(Integer.parseInt(strNum), password)) {
				response.sendRedirect("guestbook.do");
				return;
			}
	...
	/*
	 * 정상실행 후 모든 방명록 보기 - 별도의 데이터를 추가 저장할 필요없이 guestbook.do url로 요청 비정상 실행 예외
	 * error.jsp로 상황메세지 전송해서 실행
	 */
	...
	/*
	 * 정상? 비정상? 관련된 고려사항을 반영 비정상 실행일 경우 error.jsp로 이동 추가 error.jsp에 EL tag로 상황
	 * 메세지 출력
	 */
